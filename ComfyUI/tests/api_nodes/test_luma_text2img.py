...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'custom_nodes', 'luma_api'))

from luma_text2img import LumaText2Img
...
class TestLumaText2Img:
    ...
    def test_create_generation_request_basic(self, luma_node):
        ...
        payload = luma_node.create_generation_request(
            prompt="test prompt",
            negative_prompt="",
            width=1024,
            height=1024,
    ...
    def test_send_generation_request_success(self, mock_post, mock_env_with_key):
        ...
        result = mock_env_with_key.send_generation_request(payload)
    ...
    def test_send_generation_request_failure(self, mock_post, mock_env_with_key):
        """Test API request failure with retry logic"""
        # Mock failed response
        mock_post.side_effect = requests.exceptions.RequestException("Network error")
        
        payload = {"prompt": "test", "width": 1024, "height": 1024}
        
        with pytest.raises(RuntimeError, match="Failed to send request to Luma API"):
            mock_env_with_key.send_generation_request(payload)
    ...
    def test_poll_for_completion_success(self, mock_get, mock_env_with_key):
        ...
        result = mock_env_with_key.poll_for_completion("test_id")
    ...
    def test_poll_for_completion_failed(self, mock_get, mock_env_with_key):
        ...
        with pytest.raises(RuntimeError, match="Generation failed"):
            mock_env_with_key.poll_for_completion("test_id")
    ...
    def test_download_image_success(self, mock_get, luma_node):
        ...
        result = luma_node.download_image("https://example.com/image.jpg")
    ...
    def test_download_image_failure(self, mock_get, luma_node):
        ...
        with pytest.raises(RuntimeError, match="Failed to download image"):
            luma_node.download_image("https://example.com/image.jpg")
    
    @patch('luma_text2img.LumaText2Img.send_generation_request')
    @patch('luma_text2img.LumaText2Img.poll_for_completion')
    @patch('luma_text2img.LumaText2Img.download_image')
    ...
    def test_node_registration(self):
        ...
        from luma_text2img import NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS
