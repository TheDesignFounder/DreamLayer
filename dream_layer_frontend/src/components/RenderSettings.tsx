  ...
  return (
    <div className="space-y-4">
      ...
      <h4 className="mb-2 mt-6 text-sm font-bold text-[#2563EB]">
        {showResizeMode ? `2. ${t('txt2img.samplingSettings')}` : `2. ${t('txt2img.samplingSettings')}`}
      </h4>

      {/* Sampling Method Section */}
      <div className="mb-4">
        <label htmlFor="renderStyle" className="mb-2 block text-sm font-medium">{t('txt2img.samplingMethod')}</label>
        ...
        <div className="mt-4 mb-4 text-sm text-muted-foreground mx-1">
          <span className="font-medium text-[#64748B]">{t('txt2img.aboutTheMethod')}:</span> {selectedStyle.description} {t('txt2img.speed')}: {selectedStyle.speed}.
        </div>
      </div>
      ...
      {showResizeMode && (
        <div className="mb-4">
          <Slider
            ...
            step={0.01}
            defaultValue={0.55}
            label={t('txt2img.denoisingStrengthLabel')}
            sublabel=""
          />
        </div>
      )}
      ...
      {showResizeMode && (
        <>
          <h4 className="mb-2 mt-6 text-sm font-bold text-[#2563EB]">3. {t('txt2img.resizeMode')}</h4>
            ...
            >
              <div className="relative">
                <Card className={`flex items-center gap-3 p-3 cursor-pointer border ${resizeMode === "just-resize" ? "border-blue-600 bg-blue-50/50" : "border-gray-200"}`}>
                  ...
                  <label htmlFor="just-resize" className="text-sm font-medium cursor-pointer w-full">{t('txt2img.justResize')}</label>
                </Card>
              </div>
              ...
              <div className="relative">
                <Card className={`flex items-center gap-3 p-3 cursor-pointer border ${resizeMode === "crop-resize" ? "border-blue-600 bg-blue-50/50" : "border-gray-200"}`}>
                  ...
                  <label htmlFor="crop-resize" className="text-sm font-medium cursor-pointer w-full">{t('txt2img.cropAndResize')}</label>
                </Card>
              </div>
              ...
              <div className="relative">
                <Card className={`flex items-center gap-3 p-3 cursor-pointer border ${resizeMode === "resize-fill" ? "border-blue-600 bg-blue-50/50" : "border-gray-200"}`}>
                  ...
                  <label htmlFor="resize-fill" className="text-sm font-medium cursor-pointer w-full">{t('txt2img.resizeAndFill')}</label>
                </Card>
              </div>
              ...
              <div className="relative">
                <Card className={`flex items-center gap-3 p-3 cursor-pointer border ${resizeMode === "just-resize-latent" ? "border-blue-600 bg-blue-50/50" : "border-gray-200"}`}>
                  ...
                  <label htmlFor="just-resize-latent" className="text-sm font-medium cursor-pointer w-full">{t('txt2img.justResizeLatent')}</label>
                </Card>
              </div>
            </RadioGroup>
          </div>
