...
def handle_txt2img():
    ...
    try:
        ...
        if data:
            ...
            print(
                f"Negative Prompt: {data.get('negative_prompt', 'Not provided')}")
            ...
            print("-"*20)
            print(
                f"ControlNet enabled: {controlnet_data.get('enabled', False)}")
            if controlnet_data.get('units'):
                for i, unit in enumerate(controlnet_data['units']):
                    print(f"Unit {i}:")
                    print(f"  Enabled: {unit.get('enabled', False)}")
                    print(
                        f"  Has input_image: {unit.get('input_image') is not None}")
                    print(
                        f"  Input image type: {type(unit.get('input_image'))}")
                    if unit.get('input_image'):
                        print(
                            f"  Input image length: {len(unit['input_image']) if isinstance(unit['input_image'], str) else 'N/A'}")
                        print(
                            f"  Input image preview: {unit['input_image'][:50] if isinstance(unit['input_image'], str) else 'N/A'}...")
            else:
...
if __name__ == "__main__":
    ...
    # Use waitress for production-like serving and to set a higher timeout
    try:
        from waitress import serve
        # Set waitress to 30 minute timeout (1800 seconds) to allow for very slow CPU jobs
        # This prevents the server from timing out before image generation completes
        serve(app, host='127.0.0.1', port=5001, threads=1,
              connection_limit=10, asyncore_use_poll=True, channel_timeout=1800)
    except ImportError:
        print("[WARNING] waitress not installed, falling back to Flask dev server. Install waitress for production use and timeout support.")
